...
__all__ = ["build_hrnet_fpn_backbone", "build_resnet_fpn_backbone", "build_retinanet_resnet_fpn_backbone", "FPN"]
...
def build_hrnet_fpn_backbone(cfg, input_shape: ShapeSpec):
    ...
    neck = HRFPN(cfg)
    backbone = HRNet_HRFPN(bottom_up, neck)
...
class HRFPN(Backbone):
    ...
    def __init__(self, cfg):
        ...
        self.reduction_conv = nn.Sequential(
            Conv2d(in_channels=sum(self.in_channels),
                   out_channels=self.out_channels,
                   kernel_size=1),
        )
        self.fpn_conv = nn.ModuleList()
        for i in range(self.num_outs):
            self.fpn_conv.append(Conv2d(
...
class HRNet_HRFPN(Backbone):
    def __init__(self, bottom_up, neck):
        super(HRNet_HRFPN, self).__init__()
        self.bottom_up = bottom_up
        self.neck = neck
        self._size_divisibility = 32

        self._out_features = ['p2', 'p3', 'p4', 'p5', 'p6']
        self._out_feature_channels = {'p2': 256, 'p3': 256, 'p4': 256, 'p5': 256, 'p6': 256}
        self._out_feature_strides = {'p2': 4, 'p3': 8, 'p4': 16, 'p5': 32, 'p6': 64}

    def forward(self, inputs):
        output = self.bottom_up(inputs)
        output = self.neck(output)
        return {f: res for f, res in zip(self._out_features, output)}
